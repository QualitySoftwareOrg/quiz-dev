...
class UsuarioService {
  ...
  async login(email, senha) {
    ...
    if (
      usuario.tempo_bloqueio &&
      new Date(usuario.tempo_bloqueio) > new Date()
    ) {
      const minutosRestantes = Math.ceil(
        (new Date(usuario.tempo_bloqueio) - new Date()) / 60000
      );
      throw {
        status: 403,
        message: `Usuario bloqueado. Tente novamente em ${minutosRestantes} minutos.`,
      };
    }
    ...
    if (!senhaValida) {
      ...
      if (tentativasAtuais >= MAX_TENTATIVAS) {
        ...
        throw {
          status: 403,
          message: `Usuario bloqueado por ${TEMPO_BLOQUEIO_MINUTOS} minutos devido a muitas tentativas.`,
        };
      }

      throw {
        status: 401,
        message: `Email ou senha incorretos. Tentativas restantes: ${
          MAX_TENTATIVAS - tentativasAtuais
        }`,
      };
    }
